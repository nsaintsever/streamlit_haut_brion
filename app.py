...
st.set_page_config(page_title="1GCC - Wine Analytics", page_icon="🍇", layout="wide")
...
# -----------------------------
# 1) Taux en live via Boursorama (si tu veux le garder)
# -----------------------------
...
if live_conversion_rate is None:
    live_conversion_rate = 1.2
    st.warning("Taux de conversion GBP -> EUR par défaut utilisé (1.2).")

# -----------------------------
# 2) Taux historiques via yfinance -> fx_series_full (Series 1D)
# -----------------------------
ticker = "GBPEUR=X"

# Téléchargement des taux
fx_data = yf.download(ticker, start="2024-01-01", auto_adjust=False)

# L'index est déjà la date, on la normalise (on vire l'heure)
fx_data.index = pd.to_datetime(fx_data.index).normalize()

# Récupérer la colonne "Close"
fx_close = fx_data["Close"]

# ⚠️ Très important : si yfinance renvoie un DataFrame (multi-index colonnes),
# on prend explicitement la première/unique colonne pour obtenir une Series 1D.
if isinstance(fx_close, pd.DataFrame):
    # Par ex. colonnes = ['GBPEUR=X'], on prend la 1ère
    fx_series = fx_close.iloc[:, 0]
else:
    fx_series = fx_close

# Optionnel : renommer pour être propre
fx_series.name = "fx_rate"
...
# -----------------------------
# 4) Construction de la série complète fx_series_full
# -----------------------------
all_dates_fx = pd.date_range(
    start=df["date"].min().normalize(),
    end=df["date"].max().normalize(),
    freq="D",
)

fx_series_full = fx_series.reindex(all_dates_fx, method="ffill")
fx_series_full.index.name = "date"



# NE PAS remettre d'assert ici, on force nous‑mêmes une Series 1D ci‑dessus
# -----------------------------
...
vin_display_names = {}
# 1) on enlève les vin_clean NaN d’emblée
for vin_clean in df['vin_clean'].dropna().unique():  
    # 2) on ne garde que les vin non-null pour ce vin_clean
    associated_vins = (
        df.loc[df['vin_clean'] == vin_clean, 'vin']
          .dropna()
          .tolist()
    )
    # 3) si la liste est vide, on passe au vin_clean suivant
    if not associated_vins:
        continue
...
# 5) vos options pour le selectbox
...
if page == "Analyse Château - Prix & Volume":
    ...
    selected_timeframe = st.sidebar.selectbox("Choisir un *timeframe*", list(timeframe_options.keys()), index=2)
    ...
    # Appliquer le taux historique du jour
    filtered_df['conversion_rate'] = filtered_df['date'].map(fx_series_full)

    # Si jamais il y a des trous (dates hors plage, etc.)
    ...
    filtered_df['prix_eur_equiv'] = filtered_df['prix_unitaire'] * filtered_df['conversion_rate']
    ...
    df_vin_agg_full['conversion_rate'] = (
        df_vin_agg_full['date']
        .map(fx_series_full)
        .fillna(method='ffill')
        .fillna(method='bfill')
    )
    ...
    with col1:
        ...
        # 2) Ligne des prix min en EUR + remplissage de l'aire jusqu'à la courbe GBP
        fig_price.add_trace(go.Scatter(
            x=df_stats['date'],
            ...
            fill='tonexty',  # remplit l'aire entre cette courbe et la précédente
            fillcolor='rgba(0, 200, 0, 0.2)',  # liseré/aire verte semi-transparente
            hovertemplate=(
                '<b>Date:</b> %{x|%Y-%m-%d}<br>'
    ...
    with col2:
        ...
        date_selected_datetime = pd.to_datetime(date_selected).normalize()

        # Garder conversion_rate pour pouvoir faire l'équivalent EUR
        df_stock = df_vin_agg_full[df_vin_agg_full['date'] == date_selected_datetime].copy()

        # Enlever les négociants avec 0 stock
        df_stock = df_stock[df_stock['quantite_totale'] > 0]

        # Enlever les lignes sans prix
        df_stock = df_stock.dropna(subset=['prix_unitaire'])

        # Calculer le prix EUR avec le taux historique du jour
        df_stock['prix_eur_equiv'] = (df_stock['prix_unitaire'] * df_stock['conversion_rate']).round(2)

        # Préparer les colonnes à afficher
        df_stock = df_stock[['negociant', 'quantite_totale', 'prix_unitaire', 'prix_eur_equiv']]
        ...
        # (CSS + affichage identiques à ce que tu fais déjà)
        st.markdown("""
            <style>
            .center-table {
                display: flex;
